...
export default function ComerciosScreen() {
  ...
  const load = useCallback(
    async (opts?: { silent?: boolean }) => {
      ...
      try {
        ...
        setError(getUserFacingErrorMessage(err, "No se pudieron cargar los comercios"));
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
  ...
  if (authLoading || (loading && items.length === 0 && !error)) {
    return <ScreenState loading message="Cargando comercios…" />;
  }

  return (
    <View style={styles.wrap}>
      <Text style={styles.title}>Comercios</Text>
      <Text style={styles.subtitle}>{items.length} comercios</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !loading ? <Text style={styles.empty}>No hay comercios.</Text> : null
        }
        renderItem={({ item }) => (
